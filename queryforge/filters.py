# Наборы фильтров: декларативные eq/gte/contains и Pydantic-валидация.
...
# --- Операции фильтра (декларация полей) ------------------------------------
...
class _IlikeContains(_FilterOp):
    """Подстрока: ILIKE %value% (для str); для Enum — value.value в шаблоне."""
...
def _ensure_optional(ann: Any) -> Any:
    """Делает поле «опциональным» для Pydantic: ``T | None``."""
    if ann is None:
        return Any | None
    o = get_origin(ann)
...
def _filter_field_annotation_from_samodel(model_t: type[Any], col_attr: Any) -> Any:
    """Тип из ``Mapped[T]`` на ORM-модели -> ``T | None`` (через ``get_type_hints``)."""
    try:
        key = col_attr.key
...
def _pydantic_filterset_type_arg(base: type) -> type[Any] | None:
    """Pydantic v2: ``FilterSet[User]`` — отдельный класс, не ``typing.GenericAlias``."""
        ...
        **kwargs: Any,
        ...
        for k, v in list(namespace.items()):
            ...
            if k not in ann:
                ...
                # ``from __future__ import annotations`` — строка; Pydantic разрешит сам
                pass
            else:
...
class FilterSet(BaseModel, Generic[M], metaclass=_FilterSetMeta):
    """Пользователь: ``class UserFilters(FilterSet[User]): s: T | None = eq(User.s)``."""

    # Не model fields (ClassVar) — Pydantic не пытается валидировать

# Динамическая сортировка: asc/desc, from_param, default, alias, nulls, PK fallback.
...
class SortSetBase(Generic[M]):
    """База для декларативного набора сортировок."""
    ...
    def __init_subclass__(cls, **kwargs: Any) -> None:
        ...
        for n, v in cls.__dict__.items():
            ...
            if n in taken_aliases:
                msg = f"Конфликт имени поля сортировки и alias: {n!r}"
                raise ValueError(msg)
            sorts[n] = v
            taken_aliases.add(n)
            for a in v.aliases:
                if not a or a in sorts or a in taken_aliases:
                    msg = f"Дублирующийся или пустой alias сортировки: {a!r} (поле {n!r})"
                    raise ValueError(msg)
    ...
    def _resolve_sort_field(cls, name: str) -> _SortField:
        ...
        raise ValueError(f"Неизвестное поле сортировки: {name!r}. Доступны: {sorted(cls._sorts)}")
    ...
    def from_param(cls, param: str) -> list[Any]:
        """Разбор ``sort`` query: ``-created_at,email`` (несколько полей через запятую).

        Пустая строка подставляет :py:attr:`__default_sort__`, если задана.
        В конец добавляются колонки первичного ключа модели (если их ещё нет в цепочке).
        """
        raw = (param or "").strip()
        if not raw:
...
def sort_expressions(
    ...
    """Собрать ``order_by`` по именам полей одним вызовом :meth:`SortSetBase.from_param`."""
    if not names:

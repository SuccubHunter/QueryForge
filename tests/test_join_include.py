# Join, include, selectin and joined on SQLAlchemy relationships.
...
from tests.models import Order, Profile, User, UserDetailedRead, UserRead, UserStatus
...
@pytest.mark.asyncio
async def test_project_with_join_keeps_entity_semantics(session) -> None:
    await _user_with_children(session)
    repo = Repository(session, User)

    q = repo.query().join(User.orders).project(UserRead)

    rows = await q.to_list()
    page = await q.paginate(1, 20)
...
@pytest.mark.asyncio
async def test_select_with_join_keeps_row_semantics(session) -> None:
    await _user_with_children(session)
    repo = Repository(session, User)
    rows = await repo.query().join(User.orders).select(User.id, Order.title).to_list()
    assert len(rows) == 2
...
@pytest.mark.asyncio
async def test_project_after_include_raises_clear_error(session) -> None:
    await _user_with_children(session)
    repo = Repository(session, User)
    with pytest.raises(QueryForgeError, match="only with entity result queries"):
        await repo.query().include(User.orders).project(UserRead).to_list()


@pytest.mark.asyncio
async def test_project_nested_orm_allows_include_before_projection(session) -> None:
    await _user_with_children(session)
    repo = Repository(session, User)
    rows = await (
        repo.query()
        ...
        .include(User.orders)
        .project(UserDetailedRead, nested="orm")
        .to_list()
    )
    assert len(rows) == 1
    assert isinstance(rows[0], UserDetailedRead)
    assert rows[0].profile is not None
    assert rows[0].profile.bio == "hi"
    assert {o.title for o in rows[0].orders} == {"a", "b"}


@pytest.mark.asyncio
async def test_project_nested_orm_allows_include_after_projection(session) -> None:
    await _user_with_children(session)
    repo = Repository(session, User)
    rows = await (
        repo.query()
...
@pytest.mark.asyncio
async def test_project_nested_orm_missing_eager_loading_raises_projection_error(session) -> None:
    await _user_with_children(session)
    repo = Repository(session, User)
    with pytest.raises(QueryForgeError, match="nested='orm'"):
        await repo.query().project(UserDetailedRead, nested="orm").to_list()


@pytest.mark.asyncio
async def test_project_nested_orm_paginate(session) -> None:
    await _user_with_children(session)
    repo = Repository(session, User)
    page = await (
        repo.query()
    ...
    assert page.total == 1
    assert isinstance(page.items[0], UserDetailedRead)


@pytest.mark.asyncio
async def test_count_nested_orm_with_join_counts_distinct_entities(session) -> None:
    await _user_with_children(session)
    repo = Repository(session, User)
    count = await (
        repo.query()
...
@pytest.mark.asyncio
async def test_select_after_include_raises_clear_error(session) -> None:
    await _user_with_children(session)
    repo = Repository(session, User)
    with pytest.raises(QueryForgeError, match="only with entity result queries"):
        await repo.query().include(User.orders).select(User.email).to_list()
